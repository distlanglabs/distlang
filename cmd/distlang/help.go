...
func commandHelpDeployDebug() {
	fmt.Println("deploy debug - Check hosted deploy readiness without publishing")
	fmt.Println("Usage: distlang deploy debug <file>")
	fmt.Println("Checks build output, login session, service token minting, service token introspection, store auth, and deployments API access.")
}
